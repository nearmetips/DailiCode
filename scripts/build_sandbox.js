...
console.log('packing daili-code ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'daili-code-*.tgz'), { force: true });
execSync(
  `npm pack -w daili-code --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing daili-code-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'daili-code-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w daili-code-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `daili-code-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `daili-code-core-${packageVersion}.tgz`),
  0o755,
);
